...
from reports.factory import LocationFactory, SaleFactory, StoreFactory
...
def test_sales_report(setup_sales):
    ...
    The resulting UI would look something like:
    ...
    Hint: generate_series() is your friend here.
    """
...
def test_sales_report_by_fortnight(setup_sales):
    """
    The above report is all very well and good but management want to see sales
    based on their fortnightly business cycles.

    They're also not happy about having Melbourne stores appear in the list before Sydney
    stores are they're headquartered in Sydney xD.
...
def test_render_sales_report_by_fortnight(client, setup_sales):
    """
    Management are keen to see your Django & HTML prowess…

    Impress management with with your Django & HTML prowess… show them how
    easily you can convert the DB results into a table.
